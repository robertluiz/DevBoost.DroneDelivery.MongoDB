...
namespace Devboost.DroneDelivery.Mongo
{
    public class PedidosRepository : IPedidosRepository
    {
        ...
        {
            var filter = _filterDef.Eq(_ => _.Id, pedidoId);
        ...
        public async Task<List<PedidoEntity>> GetByDroneID(Guid droneId)
        {
            ...
            var filter = _filterDef.Eq(_ => _.DroneId, droneId);
        ...
        public async Task<List<PedidoEntity>> GetByDroneIDAndStatus(Guid droneId, PedidoStatus status)
        {
            ...
            var filter = _filterDef.And(_filterDef.Eq(_ => _.DroneId, droneId), _filterDef.Eq(_ => _.Status, status.ToString()));
        ...
        public async Task<PedidoEntity> GetSingleByDroneID(Guid droneId)
        {
            ...
            var filter = _filterDef.And(_filterDef.Eq(_ => _.DroneId, droneId), _filterDef.Eq(_ => _.Status, PedidoStatus.EmTransito.ToString()));
        ...
        public async Task Atualizar(PedidoEntity pedido)
        {
            ...
            var filter = _filterDef.Eq(_ => _.Id, pedido.Id);
